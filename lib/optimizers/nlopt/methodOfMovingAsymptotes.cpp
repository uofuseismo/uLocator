...
#endif
#include <nlopt.hpp>
#include "uLocator/optimizers/nlopt/methodOfMovingAsymptotes.hpp" 
...
class MethodOfMovingAsymptotes::MethodOfMovingAsymptotesImpl
{
public:
    MethodOfMovingAsymptotesImpl(
...
/// Constructor
MethodOfMovingAsymptotes::MethodOfMovingAsymptotes() :
    ULocator::Optimizers::IOptimizer(),
    pImpl(std::make_unique<MethodOfMovingAsymptotesImpl> ()) 
{
}
...
MethodOfMovingAsymptotes::MethodOfMovingAsymptotes(
    ...
    pImpl(std::make_unique<MethodOfMovingAsymptotesImpl> (logger))
{
}

/// Maximum function evaluations
void MethodOfMovingAsymptotes::setMaximumNumberOfObjectiveFunctionEvaluations(
...
int MethodOfMovingAsymptotes::getMaximumNumberOfObjectiveFunctionEvaluations() const noexcept
{
    return pImpl->mMaximumObjectiveFunctionEvaluations;
}

/// Model tolerance
void MethodOfMovingAsymptotes::setOriginTimeTolerance(
...
double MethodOfMovingAsymptotes::getOriginTimeTolerance() const noexcept
{
    return pImpl->mOriginTimeTolerance;
}

void  MethodOfMovingAsymptotes::setLocationTolerance(
    const double tolerance)
{
    if (tolerance < 0)
    {
...
double MethodOfMovingAsymptotes::getLocationTolerance() const noexcept
{
    return pImpl->mLocationTolerance;
}

void MethodOfMovingAsymptotes::setOriginTimeSearchWindowDuration(
...
double MethodOfMovingAsymptotes::getOriginTimeSearchWindowDuration()
    const noexcept
{
    return pImpl->mTimeWindow;
}

void MethodOfMovingAsymptotes::setExtentInX(
...
void MethodOfMovingAsymptotes::setExtentInY(
    const std::pair<double, double> &extent)
{
    if (extent.second <= extent.first)
    {
...
std::pair<double, double> MethodOfMovingAsymptotes::getExtentInX() const
{
...
std::pair<double, double> MethodOfMovingAsymptotes::getExtentInY() const
{
...
/// Locates 
void MethodOfMovingAsymptotes::locate(
    ...
    const IOptimizer::Norm norm)
{
    ...
    // Figure out the MMA - do I want to add CCSAQ?
    auto mmaAlgorithm = nlopt::LD_MMA;

    // Will use origin information after convergence
    Origin origin;
    if (locationProblem == IOptimizer::LocationProblem::ThreeDimensionsAndTime)
    {
        pImpl->mLogger->debug("Initializing MMA 3D and time problem...");
        ::NLOptProblem3DAndTime optimizer(mmaAlgorithm);
        ...
        {
            auto errorMessage = "3D and time MMA failed with: "
                              + std::string {e.what()};
    ...
    {
        pImpl->mLogger->debug("Initializing MMA free surface problem...");
        ::NLOptProblem2DAndTimeAndDepthAtFreeSurface optimizer(mmaAlgorithm);
        optimizer.mNorm = norm;
        optimizer.mTravelTimeCalculatorMap
            = ULocator::Optimizers::IOptimizer::getTravelTimeCalculatorMap();
        optimizer.mTopography
        ...
        {
            auto errorMessage = "Fixed to surface and time MMA failed with: "
                              + std::string {e.what()};
            pImpl->mLogger->error(errorMessage);
            throw std::runtime_error(errorMessage);
        }
    ...
    {
        pImpl->mLogger->debug("Initializing MMA fixed-depth problem...");
        ::NLOptProblem2DAndTimeAndFixedDepth optimizer(mmaAlgorithm);
        optimizer.mNorm = norm;
        optimizer.mTravelTimeCalculatorMap
            = ULocator::Optimizers::IOptimizer::getTravelTimeCalculatorMap();
        // Boundaries
        ...
        {
            auto errorMessage = "Fixed depth and time MMA failed with: "
                              + std::string {e.what()};
            pImpl->mLogger->error(errorMessage);
            throw std::runtime_error(errorMessage);
        }
...
/// Destructor
MethodOfMovingAsymptotes::~MethodOfMovingAsymptotes() = default;

/// Have origin?
bool MethodOfMovingAsymptotes::haveOrigin() const noexcept
{
...
int
MethodOfMovingAsymptotes::getNumberOfObjectiveFunctionEvaluations() const
{
...
int MethodOfMovingAsymptotes::getNumberOfGradientEvaluations() const
{
    if (!haveOrigin()){throw std::runtime_error("Event not located");}
...
double MethodOfMovingAsymptotes::getOptimalObjectiveFunction() const
{
    if (!haveOrigin()){throw std::runtime_error("Event not located");}
